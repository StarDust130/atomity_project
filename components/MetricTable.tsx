    ...
    >
      <div className="relative overflow-x-auto pb-4 pt-2 -mx-4 px-4 sm:mx-0 sm:px-0 scroll-smooth custom-scrollbar">
        <div className="min-w-[800px] lg:min-w-full">
          {/* Header Row */}
          <div className="grid grid-cols-[140px_repeat(7,minmax(90px,1fr))] sm:grid-cols-[180px_repeat(7,1fr)] gap-4 mb-2 pb-3 border-b border-(--border-subtle) font-semibold text-(--text-muted) px-2 sm:px-5 tracking-wide text-[11px] sm:text-xs uppercase shrink-0 min-w-max sm:min-w-0">
            <div>Resource</div>
          ...
          {/* Data Rows */}
          <div className="flex flex-col gap-2.5 sm:gap-3 min-w-max sm:min-w-0 pb-2">
            {data.map((metric) => (
              <motion.div
                key={metric.id}
                variants={rowVariants}
                whileHover={{ y: -2, scale: 1.005 }}
                className="group grid grid-cols-[140px_repeat(7,minmax(90px,1fr))] sm:grid-cols-[180px_repeat(7,1fr)] gap-4 py-3 sm:py-4 items-center px-4 sm:px-5 cursor-pointer transition-all duration-300 rounded-[16px] bg-white/70 backdrop-blur-sm hover:bg-white hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-white/80 hover:border-white shadow-[0_2px_10px_rgb(0,0,0,0.02)]"
              >
                <div className="font-bold flex items-center gap-2.5 sm:gap-4 text-gray-800 whitespace-nowrap leading-tight text-ellipsis overflow-hidden">
                  ...
                  <div className="bg-gray-100/80 p-1.5 sm:p-2 rounded-xl group-hover:bg-green-50 group-hover:text-(--brand-base) transition-colors duration-300 text-gray-400 shrink-0">
                    <Server className="w-4 h-4 shrink-0 transition-colors" />
                  </div>
                  <span className="min-w-0 truncate font-extrabold text-[13px] sm:text-[15px] text-gray-800">
                    {metric.name}
                  </span>
                </div>
                ...
                <div className="text-right font-black text-gray-900 text-sm sm:text-base bg-clip-text text-transparent bg-gradient-to-r from-gray-900 to-gray-600 group-hover:from-(--brand-dark) group-hover:to-(--brand-base) transition-colors duration-300 pr-4 sm:pr-0">
